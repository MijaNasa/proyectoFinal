...
const submit = () => {
    if (isEditing.value) {
        form.put(route('categorias.update', form.id), {
            onSuccess: () => {
...
const handleSearch = () => {
    ...
    window.location.href = route('categorias.index', { search: search.value });
};
</script>
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="text-3xl font-black leading-tight text-white tracking-tighter uppercase">
                    Gestión de <span class="text-brand-red italic">Categorías</span>
                </h2>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-brand-red/10 border-b border-brand-red/20">
                                ...
                                <th class="p-4 font-bold uppercase text-xs tracking-wider text-brand-red">Descripción</th>
                                <th class="p-4 font-bold uppercase text-xs tracking-wider text-brand-red text-right">Acciones</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-white/5">
                            <tr v-for="categoria in categorias.data" :key="categoria.id" class="hover:bg-white/[0.02] transition-colors">
                                <td class="p-4 font-medium">{{ categoria.nombre }}</td>
                                <td class="p-4 text-white/50 text-sm italic">{{ categoria.descripcion || 'Sin descripción' }}</td>
                                <td class="p-4 text-right">
                                    <div class="flex justify-end gap-2">
                                        <button 
                                            @click="openModal(categoria)"
                                            class="p-2 text-white/50 hover:text-brand-red transition-colors"
                                            title="Editar"
                                        >
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                            ...
                            <tr v-if="categorias.data.length === 0">
                                <td colspan="3" class="p-12 text-center text-white/30 italic">
        ...
        <div v-if="showModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-md card border-brand-red/20 shadow-2xl shadow-brand-red/10 overflow-hidden transform transition-all">
                <div class="bg-brand-red/10 p-4 border-b border-brand-red/20 flex justify-between items-center">
                    <h3 class="text-xl font-bold uppercase tracking-tight">
                        {{ isEditing ? 'Editar' : 'Nueva' }} <span class="text-brand-red italic">Categoría</span>
                    </h3>
                ...
                <form @submit.prevent="submit" class="p-6">
                    <div class="space-y-4">
                        <div>
                            ...
                            <div v-if="form.errors.nombre" class="text-brand-red text-xs mt-1">{{ form.errors.nombre }}</div>
                        </div>
                        <div>
                            <label class="block text-xs font-bold uppercase tracking-widest text-white/50 mb-1">Descripción</label>
                            <textarea v-model="form.descripcion" class="input-field w-full h-32"></textarea>
                            <div v-if="form.errors.descripcion" class="text-brand-red text-xs mt-1">{{ form.errors.descripcion }}</div>
                        </div>
                    </div>

                    <div class="mt-8 flex justify-end gap-3">
                        <button type="button" @click="showModal = false" class="px-6 py-2 rounded-lg font-bold text-white/70 hover:bg-white/5 transition-colors">
                            Cancelar
                        </button>
