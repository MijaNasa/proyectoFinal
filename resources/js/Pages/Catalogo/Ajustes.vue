...
const filteredItems = computed(() => {
    ...
    } else if (estadoFiltro.value === 'inactivos') {
        list = list.filter(item => !item.activo);
    }
...
// Toggle Activo / Desactivar Logic
const itemNameForTab = (tab = currentTab.value) => {
    if (tab === 'autores') return 'Autor';
    if (tab === 'categorias') return 'Categoría';
    if (tab === 'formatos') return 'Formato';
    if (tab === 'proveedores') return 'Proveedor';
    if (tab === 'idiomas') return 'Idioma';
    return 'Registro';
};
...
const toggleActivo = (item) => {
    ...
    const singular = itemNameForTab(type);

    if (item.activo) {
        if (count > 0) {
            darkSwal.fire({
                title: 'No se puede desactivar',
                text: `Este registro tiene ${count} obra(s) asociada(s). Debe desvincularla(s) del catálogo antes de poder desactivarlo por seguridad.`,
                icon: 'error',
            });
            return;
        }
        ...
        darkSwal.fire({
            title: `¿Desactivar ${singular.toLowerCase()}?`,
            text: 'El registro se marcará como inactivo y no estará disponible para nuevas obras en el catálogo.',
            icon: 'warning',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                router.patch(route('catalogo.ajustes.toggleActivo', { type, id: item.id }), {}, {
                    preserveScroll: true,
                    onSuccess: (page) => {
                        ...
                        if (errorMsg) {
                            ...
                            darkSwal.fire({
                                title: 'Desactivado',
                                text: page.props.flash?.message || `${singular} desactivado con éxito.`,
                                icon: 'success',
                                timer: 1500,
                                showConfirmButton: false
                            });
        ...
        router.patch(route('catalogo.ajustes.toggleActivo', { type, id: item.id }), {}, {
            ...
            onSuccess: (page) => {
                darkSwal.fire({
                    title: 'Reactivado',
                    text: page.props.flash?.message || `${singular} reactivado con éxito.`,
                    icon: 'success',
                    timer: 1500,
                    showConfirmButton: false
                });
...
const itemName = computed(() => {
    return itemNameForTab(editingType.value);
});
</script>
        ...
        <div class="py-8 page-ajustes">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="bg-[#131316] border border-white/5 rounded-2xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-xl">
                    ...
                    <div class="flex items-center gap-1 bg-[#0d0d0f] p-1 rounded-xl border border-white/5">
                        <button 
                            @click="estadoFiltro = 'todos'"
                            class="px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all"
                            :class="estadoFiltro === 'todos' ? 'bg-white text-black font-bold shadow' : 'text-zinc-400 hover:text-white'"
                        >
                            Todos
                        </button>
                        <button 
                            ...
                            class="px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all"
                            :class="estadoFiltro === 'activos' ? 'bg-white text-black font-bold shadow' : 'text-zinc-400 hover:text-white'"
                        >
                            Activos
                        </button>
                        <button 
                            @click="estadoFiltro = 'inactivos'"
                            class="px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all"
                            :class="estadoFiltro === 'inactivos' ? 'bg-white text-black font-bold shadow' : 'text-zinc-400 hover:text-white'"
                        >
                            Inactivos
                        </button>
                    </div>
                            ...
                            <tbody class="divide-y divide-white/5 text-sm">
                                ...
                                <tr v-for="item in filteredItems" :key="item.id" class="hover:bg-white/[0.02] transition-colors">
                                    ...
                                    <td class="p-4 text-center">
                                        <span v-if="item.activo" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                                            <span class="w-1.5 h-1.5 rounded-full bg-emerald-400"></span>
                                            <span>Activo</span>
                                        </span>
                                        <span v-else class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-zinc-500/10 text-zinc-400 border border-zinc-500/20">
                                    ...
                                    <td class="p-4 text-center w-36">
                                        <div class="flex justify-center gap-1 items-center">
                                            ...
                                            <button 
                                                ...
                                                :title="'Desactivar ' + itemNameForTab(currentTab)"
                                            >
                                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            ...
                                            <button 
                                                ...
                                                :title="'Reactivar ' + itemNameForTab(currentTab)"
                                            >
                                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
