        ...
        <aside class="hidden md:flex flex-col w-64 bg-brand-surface border-r border-white/5 z-50 transition-all duration-300">
            ...
            <div class="h-16 flex items-center px-6 border-b border-white/5">
                <Link :href="route('dashboard')" class="flex items-center gap-2">
                    ...
                    <span class="text-xl font-black tracking-tighter text-white italic">PURO<span class="text-brand-red">COMIC</span></span>
                </Link>
            </div>
            ...
            <div class="flex-1 overflow-y-auto py-6 space-y-2 px-4 scrollbar-hide">
                ...
                <div class="space-y-1">
                    ...
                    <div v-show="expandedGroups.operations" class="pl-4 space-y-1">
                        <DropdownLink :href="route('ventas.index')" :active="route().current('ventas.*')" class="block py-2 text-[10px] font-bold uppercase text-white/50 hover:text-brand-red">Nueva Venta / Historial</DropdownLink>
                        <DropdownLink :href="route('cierre-cajas.index')" :active="route().current('cierre-cajas.*')" class="block py-2 text-[10px] font-bold uppercase text-white/50 hover:text-brand-red">Cierres de Caja</DropdownLink>
                    </div>
                </div>
                ...
                <div class="space-y-1">
                    ...
                    <div v-show="expandedGroups.people" class="pl-4 space-y-1">
                        <DropdownLink :href="route('clientes.index')" :active="route().current('clientes.*')" class="block py-2 text-[10px] font-bold uppercase text-white/50 hover:text-brand-red">Clientes</DropdownLink>
                        <DropdownLink :href="route('empleados.index')" :active="route().current('empleados.*')" class="block py-2 text-[10px] font-bold uppercase text-white/50 hover:text-brand-red">Recursos Humanos</DropdownLink>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6 border-t border-white/5 bg-black/20">
                <Dropdown align="top-right" width="48">
                    <template #trigger>
                        <button class="w-full flex items-center gap-3 group">
                            ...
                            <div class="flex-1 text-left overflow-hidden">
                                ...
                                <div class="text-[8px] font-bold text-white/30 uppercase tracking-tighter truncate">Operador Activo</div>
                            </div>
                        </button>
                    </template>
                    <template #content>
                        <div class="bg-brand-surface border border-white/10 rounded-md overflow-hidden">
                            <DropdownLink :href="route('profile.edit')" class="hover:bg-brand-red/10 hover:text-brand-red transition-colors italic font-bold">Ajustes de Perfil</DropdownLink>
                            <DropdownLink :href="route('logout')" method="post" as="button" class="hover:bg-brand-red/10 hover:text-brand-red transition-colors italic font-bold">Cerrar Sesión</DropdownLink>
                        </div>
                    </template>
                </Dropdown>
            </div>
